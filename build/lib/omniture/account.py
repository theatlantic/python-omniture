...
class Suite(Value):
    ...
    def segments(self):
        ...
        if self.account.cache:
            owned = self.request_cached('Segments', 'Get',{'accessLevel':'owned'})
            shared = self.request_cached('Segments', 'Get',{'accessLevel':'shared'})
            data = owned + shared
        else:
            owned = self.request('Segments', 'Get', {'accessLevel':'owned'})
            shared = self.request('Segments', 'Get', {'accessLevel':'shared'})
            data = owned + shared
